...
impl Clone for PipelineCache {
    fn clone(&self) -> Self {
        Self { device: self.device.clone(), activation_forward_pipelines: RwLock::new(self.activation_forward_pipelines.read().unwrap().clone()), 
            activation_backward_pipelines: RwLock::new(self.activation_backward_pipelines.read().unwrap().clone()), 
            multiplication_pipeline: self.multiplication_pipeline.clone(), backp_multiplication_pipeline: self.backp_multiplication_pipeline.clone(), 
            backp_weight_bias_pipeline: self.backp_weight_bias_pipeline.clone(), apply_gradient_pipeline: self.apply_gradient_pipeline.clone() }
    }
}
