use super::activation::{ActivationType, ActivationParams};
...
impl NNLayer {
    pub fn new(
        ...
        let biases_data: Vec<f32> = (0..size).map(|_| rand::random::<f32>()/100. - 0.005).collect();
        let biases = GpuTensor::from_data(&biases_data, device);
        
        let weights_data: Vec<f32> = (0..size * prev_size).map(|_| rand::random::<f32>()/100. - 0.005).collect();
    ...
    pub fn copy_weights_to(&self, dst: &Self, encoder: &mut wgpu::CommandEncoder) {
        match (self, dst) {
            (
                NNLayer::FullyConnectedLayer {
                    weights: src_w,
                ...
                encoder.copy_buffer_to_buffer(
                    ...
                    &dst_w.buffer,
                    0,
                    (src_w.len * std::mem::size_of::<f32>()) as u64,
                );
                ...
                encoder.copy_buffer_to_buffer(
                    ...
                    &dst_b.buffer,
                    0,
                    (src_b.len * std::mem::size_of::<f32>()) as u64,
                );
            }
