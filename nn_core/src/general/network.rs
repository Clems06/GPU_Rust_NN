...
impl Network {
    pub fn new(device_arc: Arc<wgpu::Device>, queue: wgpu::Queue, batch_size: u32, topology: &[u32], activations: &[ActivationType], loss: LossType) -> anyhow::Result<Self>{
        ...
        for i in 1..topology.len() {
            ...
            if activation==ActivationType::SoftMax {
                if i != topology.len() { panic!("Activation SoftMax only allowed in last layer"); }
                else if loss != LossType::CrossEntropy { panic!("Activation SoftMax requires CrossEntropy loss");}
            } else if i == topology.len() && loss == LossType::CrossEntropy { panic!("CrossEntropy loss requires SoftMax activation in last layer");}

            

            layers.push(NNLayer::new(&device_arc, topology[i as usize], topology[(i-1) as usize], batch_size, activation,));
        }

        let loss_pipeline = loss.get_pipeline(&device_arc);
        
        
        let pipeline_cache = PipelineCache::new(device_arc.clone());

        Ok(Self{layers, batch_size, device: device_arc, queue, loss_pipeline, pipeline_cache, loss_type: loss, topology: topology.into()})
    }
    ...
    pub fn get_output(&self, input: &GpuTensor) -> Vec<f32> {
    ...
    pub fn copy_weights_to(&self, target: &Network, device: &wgpu::Device, queue: &wgpu::Queue) {
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
        for (src_layer, dst_layer) in self.layers.iter().zip(target.layers.iter()) {
            src_layer.copy_weights_to(dst_layer, &mut encoder);
        }
