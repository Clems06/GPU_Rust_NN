...
use nn_core::general::{activation::{self, ActivationType}, network, tensor, loss::LossType};
mod mnist;
mod download_mnist;
...
async fn main() {
    ...
    let network = match network::Network::new(std::sync::Arc::new(device), queue, batch_size, &topology, &activations, LossType::CrossEntropy) {
        Ok(net) => net,
        Err(e) => {
    ...
    for epoch in 0..epochs {
        ...
        for test_batch in 0..test_batches {
            ...
            let predictions = network.get_output(&test_inputs_gpu);
        ...
        if epoch%20==0{
            ...
            for train_batch in 0..batches_per_epoch {
                ...
                let predictions = network.get_output(&inputs_gpu);
    ...
    for i in 0..5 {
        ...
        let output = network.get_output(&input_gpu);
